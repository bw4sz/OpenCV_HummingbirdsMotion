...
def run(fP,accAvg,threshL):
	...
	if plotwatcher:
		...
		####Adaptively set the aggregate threshold, we know that about most of data are negatives. 
		...
		if adapt:
			...
			if frame_count % fift == 0:  
				...
				print(str(frame_count) + " accAvg is changed to: " + str(accAvg))
				
				#Write change to log file
				log_file.write(str(frame_count) + " accAvg is changed to: " + str(accAvg) + "\n" )
				...
				if floor == 1 :
					accAvg=floorvalue
					print(str(frame_count) + " accAvg is reset to: " + str(accAvg))
					#Write change to log file
					log_file.write(str(frame_count) + " accAvg is reset to: " + str(accAvg) + "\n" )
               ...
               #Merge boxes
                ...
                ##################################################
                #Write image to file
                cv2.imwrite(str(frame_count)+".jpg",camera_imageO)
...
######################################################################################################
#Running in the bisque environment, we use a parser of the xml document to get the argument settings
######################################################################################################
import optparse
